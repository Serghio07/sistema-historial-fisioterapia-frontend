...
import { progresoHistoria, tituloHistoria } from '../utils/agruparCitas';
...
function AppointmentMenu({ cita, isAdmin, onView, onEdit, onCancel, onDelete }) {
  ...
  return <FloatingActionMenu label="Acciones de la cita" buttonClassName="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50">{(close) => <>
    ...
    {isAdmin && <button type="button" role="menuitem" className="menu-action text-red-700" onClick={() => action(close, onDelete)}><Trash2 size={15} />Eliminar</button>}
  </>}</FloatingActionMenu>;
}

function AppointmentRow({ cita, isAdmin, actions }) {
  ...
  return <div className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm sm:grid-cols-[105px_125px_minmax(0,1fr)_auto] sm:items-center">
    ...
    <span className="flex items-center justify-between gap-2 sm:justify-end"><Badge className={stateStyles[cita.estado] || stateStyles.Pendiente}>{cita.estado}</Badge><AppointmentMenu cita={cita} isAdmin={isAdmin} {...actions} /></span>
  </div>;
}

function AppointmentSection({ title, appointments, initiallyVisible = true, isAdmin, actions }) {
  ...
  return <section className="mt-4">
    ...
    <div className={`grid transition-[grid-template-rows] duration-200 ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}><div className="overflow-hidden"><div className="mt-2 grid gap-2">{visible.map((cita) => <AppointmentRow key={cita.id} cita={cita} isAdmin={isAdmin} actions={actions} />)}{appointments.length > 4 && <button type="button" className="justify-self-start text-xs font-black text-brand-700" onClick={() => setShowAll((value) => !value)}>{showAll ? 'Ver menos' : `Ver ${appointments.length - 4} citas más`}</button>}</div></div></div>
  </section>;
}

function HistoryGroup({ section, isAdmin, actions }) {
  ...
  return <article className="rounded-2xl border border-brand-100 bg-brand-50/20 p-4">
    ...
    <AppointmentSection title="Próximas citas" appointments={section.upcoming} isAdmin={isAdmin} actions={actions} />
    <AppointmentSection title="Citas anteriores" appointments={section.previous} initiallyVisible={false} isAdmin={isAdmin} actions={actions} />
    <AppointmentSection title="Canceladas y reprogramadas" appointments={section.cancelled} initiallyVisible={false} isAdmin={isAdmin} actions={actions} />
...
export default function ListadoCitasAgrupado({ groups, expanded, onToggle, isAdmin, onView, onEdit, onCancel, onDelete, onPatient, onNewAppointment, onClearFilters }) {
  ...
  return <div className="grid gap-4">{groups.map((group) => {
    const patient = group.paciente || {}; const open = Boolean(expanded[group.key]); const next = group.proxima;
