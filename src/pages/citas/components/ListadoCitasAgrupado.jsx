...
function AppointmentRow({ cita, isAdmin, actions }) {
  ...
  return <div className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm sm:grid-cols-[105px_125px_minmax(0,1fr)_auto] sm:items-center">
    ...
    <span className="flex items-center justify-between gap-2 sm:justify-end"><Badge className={stateStyles[cita.estado] || stateStyles.Pendiente}>{estadoCitaVisible(cita)}</Badge><AppointmentMenu cita={cita} isAdmin={isAdmin} {...actions} /></span>
  </div>;
}
