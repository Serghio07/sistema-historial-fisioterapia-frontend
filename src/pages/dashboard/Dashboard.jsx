...
function Dashboard() {
  ...
  return <section className="grid gap-6">
    {loading && <Loader />}
    <div className="dashboard-hero relative overflow-hidden rounded-2xl p-6 text-white md:p-8"><div className="relative z-10 flex flex-wrap items-end justify-between gap-6"><div><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold backdrop-blur"><Sparkles size={14} /> Resumen de la jornada</div><p className="text-sm font-medium capitalize text-teal-100">{fecha}</p><h2 className="mt-1 text-3xl font-black tracking-tight md:text-4xl">Buenos días, {nombre.split(' ')[0]}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-teal-50/80">Aquí tienes el estado de la atención clínica y las actividades prioritarias de hoy.</p></div><Link to="/citas" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-brand-900 shadow-lg transition hover:-translate-y-0.5 hover:bg-teal-50"><Plus size={18} /> Agendar cita</Link></div></div>
