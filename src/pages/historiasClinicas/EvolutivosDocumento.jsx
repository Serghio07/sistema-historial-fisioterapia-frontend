...
const inyectablesEvolucion = (evolution) => {
  const detalle = [evolution?.inyectable_nombre, evolution?.inyectable_dosis].filter(Boolean);
  return detalle.length ? detalle.join(' · ') : evolution?.inyectables || '';
};
...
export default function EvolutivosDocumento({ historia, onClose }) {
  ...
  return <Modal open title="Evoluciones y plan de tratamiento" subtitle={`${nombrePaciente(historia.paciente)} · ${evolutions.length} evoluciones registradas`} onClose={onClose} size="lg">
