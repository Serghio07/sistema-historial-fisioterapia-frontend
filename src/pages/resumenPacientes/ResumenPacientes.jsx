...
const tabs = ['Resumen', 'Sesiones', 'Asistencias', 'Pagos y deudas', 'Historias clínicas', 'Citas', 'Documentos'];
...
export default function ResumenPacientes() {
  ...
  const citas = useMemo(() => (data?.citas || []).filter((c) => inPeriod(c.fecha) && matchesSearch(`${c.fecha} ${c.motivo || ''} ${c.tipo_atencion || ''} ${c.estado || ''} ${c.registrado_por?.nombre || ''}`, recordQuery)), [data, dateFrom, dateTo, recordQuery]);
  ...
  const allAttended = (data?.sesiones || []).filter(attended);
  const attendedRows = sessions.filter(attended);
  const absences = sessions.filter(absent);
  const activeConcepts = concepts.filter((c) => c.estado !== 'Anulado');
  const movements = activeConcepts.flatMap((c) => (c.movimientos || []).filter(activeMovement).map((m) => ({ ...m, concepto: c })));
  ...
  const economic = balance <= 0 ? (total ? 'Pagado' : 'Sin deuda') : paid > 0 ? 'Parcial' : 'Pendiente';
  ...
  const exportExcel = async () => {
    ...
    sheet.addRows([
      ...
      ['Total facturado', total], ['Total pagado', paid], ['Saldo pendiente', balance], ['Estado económico', economic]
    ]);
    const sessionsSheet = workbook.addWorksheet('Sesiones');
    sessionsSheet.addRow(['Fecha', 'Historia', 'Sesión', 'Asistencia', 'Profesional', 'Estado de pago']);
    sessions.forEach((s) => sessionsSheet.addRow([s.fecha, historyName(s.historia_clinica), s.numero_sesion, s.asistencia, s.profesional_responsable || s.registrado_por?.nombre, s.estado_pago]));
  ...
  return <section className="grid gap-4">
    ...
    <div className="module-hero rounded-xl">
      <div><p className="text-sm font-bold text-brand-50">Consulta integral</p><h1 className="mt-1 text-2xl font-black">Resumen de Pacientes</h1><p className="mt-2 text-sm text-brand-50">Información clínica, asistencial y económica en una sola pantalla.</p></div>
    ...
    {!data && !loading && <Empty>Busca y selecciona un paciente para visualizar su resumen clínico y económico.</Empty>}
    ...
    {data && <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric title="Visitas" value={attendedRows.length} detail={`${allAttended.length} históricas · Última ${formatDate(lastVisit?.fecha)}`} tone="green" /><Metric title="Sesiones" value={attendedRows.length} detail={`${Math.max(0, histories.reduce((s, h) => s + Number(h.evaluacion_final?.sesiones_contratadas || 0), 0) - allAttended.length)} pendientes · ${absences.length} faltas`} /><Metric title="Pagado" value={money(paid)} detail={`${movements.length} movimientos en el periodo`} tone="green" /><Metric title="Saldo pendiente" value={money(balance)} detail={`${debtCount} deudas activas · ${economic}`} tone={economicTone} /></div>
      ...
      <div className="panel overflow-hidden p-0">
        ...
        <div className="p-4">
          {tab === 'Resumen' && <div className="grid gap-4 lg:grid-cols-2"><section className="rounded-xl border border-slate-200 p-4"><h3 className="font-black text-slate-800">Actividad clínica</h3><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Visitas</dt><dd className="font-black">{attendedRows.length}</dd></div><div><dt className="text-slate-500">Faltas</dt><dd className="font-black">{absences.length}</dd></div><div><dt className="text-slate-500">Última atención</dt><dd className="font-black">{formatDate(lastVisit?.fecha)}</dd></div><div><dt className="text-slate-500">Próxima cita</dt><dd className="font-black">{formatDate(nextAppointment?.fecha)}</dd></div></dl></section><section className="rounded-xl border border-slate-200 p-4"><h3 className="font-black text-slate-800">Actividad económica</h3><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Facturado</dt><dd className="font-black">{money(total)}</dd></div><div><dt className="text-slate-500">Pagado</dt><dd className="font-black text-emerald-700">{money(paid)}</dd></div><div><dt className="text-slate-500">Pendiente</dt><dd className="font-black text-red-700">{money(balance)}</dd></div><div><dt className="text-slate-500">Último método</dt><dd className="font-black">{movements[0]?.metodo || 'Sin pagos'}</dd></div></dl></section></div>}
          {tab === 'Sesiones' && <Table columns={['Fecha', 'Historia', 'Sesión', 'Asistencia', 'Pago', 'Profesional', 'Acciones']} rows={sessions.map((s) => [formatDate(s.fecha), historyName(s.historia_clinica), `N.º ${s.numero_sesion || '-'}`, s.asistencia || 'Pendiente', s.estado_pago || 'Pendiente', s.profesional_responsable || s.registrado_por?.nombre || 'Sin dato', <div className="flex gap-1"><ActionButton label="Ver sesión" icon={Eye} tone="view" onClick={() => go('/sesiones', { verSesionId: s.id })} />{s.historia_clinica_id && <ActionButton label="Ver historia" icon={ClipboardList} tone="edit" onClick={() => go(`/historias-clinicas/${s.historia_clinica_id}`)} />}</div>])} empty="No se encontraron sesiones para el periodo seleccionado." />}
          {tab === 'Asistencias' && <div className="grid gap-4"><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Metric title="Asistencias" value={attendedRows.length} detail="Visitas realizadas" tone="green" /><Metric title="Faltas" value={absences.length} detail="No suman como visita" tone="red" /><Metric title="Pendientes" value={sessions.filter((s) => s.asistencia === 'pendiente').length} detail="Sin atención registrada" tone="amber" /><Metric title="% asistencia" value={`${sessions.length ? Math.round(attendedRows.length / Math.max(1, attendedRows.length + absences.length) * 100) : 0}%`} detail="Sobre asistencias y faltas" /></div><div className="flex flex-wrap gap-2">{sessions.map((s) => <span key={s.id} className={`rounded-lg border px-3 py-2 text-xs font-bold ${attended(s) ? 'border-emerald-100 bg-emerald-50 text-emerald-700' : absent(s) ? 'border-red-100 bg-red-50 text-red-700' : 'border-slate-200 bg-slate-50 text-slate-500'}`}>{formatDate(s.fecha)} · {s.asistencia || 'Pendiente'}</span>)}</div></div>}
          {tab === 'Pagos y deudas' && <Table columns={['Fecha', 'Concepto', 'Historia', 'Total', 'Pagado', 'Saldo', 'Estado', 'Acciones']} rows={activeConcepts.map((c) => { const cPaid = (c.movimientos || []).filter(activeMovement).reduce((s, m) => s + Number(m.monto || 0), 0); const cBalance = Math.max(0, Number(c.monto_esperado || 0) - cPaid); return [formatDate(c.fecha_origen), c.detalle, historyName(c.historia_clinica), money(c.monto_esperado), money(cPaid), money(cBalance), cBalance <= 0 ? 'Pagado' : cPaid ? 'Parcial' : 'Pendiente', <ActionButton label="Ver pago o deuda" icon={Eye} tone="view" onClick={() => go('/control-financiero/planilla-pagos', { conceptoCobroId: c.id })} />]; })} empty="No se encontraron pagos o deudas para el periodo seleccionado." />}
