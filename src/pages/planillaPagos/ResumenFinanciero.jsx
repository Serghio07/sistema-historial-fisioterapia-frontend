...
export default function ResumenFinanciero() {
  ...
  const exportCsv=()=>{const rows=data?.movimientos_recientes||[];const content=[['Fecha','Paciente','Concepto','Método','Monto','Recibido por','Estado'],...rows.map(x=>[x.fecha,x.paciente,x.concepto,x.metodo,x.monto,x.recibido_por,x.estado])].map(row=>row.map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+content],{type:'text/csv;charset=utf-8'}));a.download=`Resumen_financiero_${range.fecha_inicio}_${range.fecha_fin}.csv`;a.click();URL.revokeObjectURL(a.href);};
