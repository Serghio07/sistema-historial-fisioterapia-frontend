...
const schema = z.object({
  ...
  descripcion: z.string().trim().min(1, 'La descripción del evolutivo es obligatoria.'),
...
function EvolutivoForm({ historias, user, initial, onClose, onSaved }) {
  ...
  const save = async (data) => {
    ...
    await Swal.fire({ icon: 'success', title: initial ? 'Evolutivo actualizado correctamente' : 'Evolutivo registrado correctamente', confirmButtonColor: '#0F766E' });
    onSaved();
  };
  ...
  return <form onSubmit={handleSubmit(save)} className="grid gap-4">
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <Field label="Descripción del evolutivo" error={errors.descripcion} className="sm:col-span-2"><textarea rows="4" className={control} placeholder="Describa la evolución clínica observada" {...register('descripcion')} /></Field>
    ...
    <div className="flex justify-end gap-2 border-t border-slate-200 pt-3"><Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={isSubmitting}>{initial ? 'Actualizar evolutivo' : 'Guardar evolutivo'}</Button></div>
  </form>;
}
...
export default function EvolutivosClinicos() {
  ...
  const patients = useMemo(() => {
    ...
    historias.forEach((historia) => {
      ...
      const items = evolutivosReales(historia, filters.estado === 'todos' || filters.estado === 'anulado').filter((e) => {
        ...
        const searchable = `${nombrePaciente(paciente)} ${paciente?.ci || ''} ${historia.diagnostico_medico || ''} ${profesionalEvolutivo(e, historia)} ${descripcionEvolutivo(e)}`.toLocaleLowerCase('es-BO');
        return (!term || searchable.includes(term)) && (!filters.from || date >= filters.from) && (!filters.to || date <= filters.to)
  ...
  const annul = async (historia, evolutivo) => {
    setMenu(null);
    const result = await Swal.fire({ icon: 'warning', title: '¿Anular evolutivo?', text: `${formatDate(fechaEvolutivo(evolutivo))} · Sesión ${numeroEvolutivo(evolutivo)}. El evolutivo dejará de mostrarse como activo, pero permanecerá en el historial clínico.`, showCancelButton: true, reverseButtons: true, confirmButtonText: 'Anular evolutivo', cancelButtonText: 'Cancelar', confirmButtonColor: '#DC2626', cancelButtonColor: '#64748B' });
    ...
    await load();
    await Swal.fire({ icon: 'success', title: 'Evolutivo anulado correctamente', confirmButtonColor: '#0F766E' });
  };
  ...
  return <section className="evolutivos-clinicos grid gap-5">
    {loading && <Loader />}
    <div className="overflow-hidden rounded-xl border border-brand-100 bg-white shadow-sm"><div className="module-hero"><div><p className="text-sm font-bold text-brand-700">Gestión clínica</p><h1 className="mt-1 text-2xl font-bold md:text-3xl">Evolutivos clínicos</h1><p className="mt-2 text-sm text-slate-500">Seguimiento de la evolución clínica de los pacientes</p></div><div className="flex flex-wrap items-center justify-end gap-3"><div className="rounded-xl border border-brand-100 bg-white/60 px-4 py-2 text-center"><strong className="block text-xl text-brand-700">{patients.length}</strong><span className="text-[10px] font-bold uppercase text-slate-500">Pacientes</span></div><div className="rounded-xl border border-brand-100 bg-white/60 px-4 py-2 text-center"><strong className="block text-xl text-brand-700">{totalEvolutivos}</strong><span className="text-[10px] font-bold uppercase text-slate-500">Evolutivos</span></div><Button onClick={() => openNew()}><Plus size={17} />Nuevo evolutivo</Button></div></div></div>

    <div className="panel rounded-2xl">
      <div className="grid gap-3 lg:grid-cols-[1fr_auto]"><div className="relative"><Activity size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por paciente, CI, diagnóstico, profesional o contenido del evolutivo" className="w-full rounded-lg border-[#CBD5E1] bg-white py-2.5 pl-11 pr-3 text-sm focus:border-brand-500 focus:ring-brand-500/20" /></div><Button variant="secondary" onClick={() => setFiltersOpen(!filtersOpen)}><Filter size={17} />Filtros avanzados</Button></div>
      ...
      <div className="hidden grid-cols-[minmax(230px,1.35fr)_100px_90px_155px_minmax(150px,.8fr)_150px_28px] gap-3 rounded-t-xl border border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-bold uppercase text-slate-500 md:grid"><span>Paciente</span><span>Historias clínicas</span><span>Evolutivos</span><span>Último evolutivo</span><span>Profesional</span><span>Acciones</span><span /></div>
      <div className="divide-y divide-slate-200 rounded-b-xl border border-t-0 border-slate-200">{visible.map((group) => {
        ...
        return <article key={group.id} className={`border transition-all duration-300 ${expanded ? 'border-teal-200 border-l-4 border-l-teal-500 bg-teal-50/50' : 'border-transparent bg-white hover:bg-teal-50/20'}`}>
          <div role="button" tabIndex={0} onClick={() => togglePatient(group.id)} onKeyDown={(e) => ['Enter', ' '].includes(e.key) && togglePatient(group.id)} className="grid cursor-pointer items-center gap-3 px-4 py-3.5 md:grid-cols-[minmax(230px,1.35fr)_100px_90px_155px_minmax(150px,.8fr)_150px_28px]">
            ...
            <span className="flex items-center gap-2"><FileText size={15} className="text-teal-600" /><span><strong className="block text-xs text-slate-800">{group.total}</strong><small className="text-[10px] text-slate-500">evolutivos</small></span></span>
        ...
        return <div className="grid max-h-[72vh] gap-3 overflow-y-auto pr-1">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="grid gap-3">
              ...
              <div className="detail-clinical-card"><ClipboardList size={22} /><div><span>Evolutivo</span><strong>{descripcionEvolutivo(current) || 'Sin procedimiento registrado'}</strong></div></div>
            </div>
