...
export async function exportSesionesSemanalesExcel({ registros, range, generatedBy }) {
  ...
  const rows = grouped.map((registro) => {
    ...
    const ultima = [...sesiones].sort((a, b) => String(b.fecha).localeCompare(String(a.fecha)))[0];
    ...
    return {
      ...
      values: [
        ...
        registro.paciente?.ci || 'Sin registrar',
        registro.paciente?.telefono || registro.telefono || 'Sin registrar',
        historiaTexto,
  ...
  const headers = ['Paciente', 'CI', 'Teléfono', 'Historia clínica', 'Diagnóstico', 'Fecha última sesión', 'Sesiones registradas', 'Asistió', 'Pendiente', 'Faltó', 'Deuda semanal', 'Fármacos', 'Estado de la historia', 'Profesional responsable'];
  ...
  rows.forEach((entry, index) => {
    ...
    row.getCell(11).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: Number(entry.values[10]) > 0 ? 'FFFFEDD5' : 'FFDCFCE7' } };
    row.getCell(12).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: entry.hasDrugs ? 'FFF3E8FF' : 'FFF1F5F9' } };
  });
  sheet.autoFilter = { from: { row: 9, column: 1 }, to: { row: 9 + rows.length, column: 14 } };

  const summaryStart = 11 + rows.length;
  sheet.mergeCells(summaryStart, 1, summaryStart, 14);
  ...
  summary.forEach(([label, value], index) => {
    ...
    if (label === 'Deuda total') row.getCell(2).numFmt = '#,##0.00 "Bs"';
  });

  const widths = [28, 16, 17, 38, 38, 20, 15, 12, 12, 12, 18, 30, 22, 32];
